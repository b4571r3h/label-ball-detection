...
- Einmalige Tisch-Kalibrierung durch Anklicken der 4 Ecken (TL, TR, BR, BL)
- Ball-Detektion via YOLO
- Projektion der Treffer auf die Tischebene (Top-Down, 2.74m x 1.525m)
- Heuristische Bounce-Erkennung (lokales Maximum der Bild-y)
- Heatmap + CSV speichern, optional: Hintergrundbild für Heatmap

Beispiel:
python infer_bounce_heatmap.py --weights runs/detect/train/weights/best.pt \
  --source "https://www.youtube.com/watch?v=_-yAPHeppn8" \
  --conf 0.25 --imgsz 640 --calib table_calib.json \
  --bg_image tisch_heatmap.jpg --bg_rotate 0 \
  --save_heatmap heatmap.png --save_csv bounces.csv --save_preview preview.mp4 --show
"""
import argparse
import json
import io
...
def pick_points(img, n=4, title="Click table corners: TL, TR, BR, BL (Enter confirms, r=reset, u=undo)"):
    ...
    # erst anzeigen, dann Callback setzen (macOS HighGUI-Workaround)
    ...
    while True:
        ...
        if key in (ord('u'), 8):   # undo
            if pts:
                pts.pop()
        if key == ord('r'):        # reset
            pts = []
        if len(pts) >= n and key in (13, 10, ord('y')):  # Enter/Return/'y'
            break
...
def find_bounces(track, fps, proj_pts, confs, min_gap_s=0.25):
    ...
    for i in range(1, len(track)-1):
        if ys[i-1] < ys[i] > ys[i+1]:  # lokales Maximum
            if confs[i] < 0.15:
                continue
            xy_m = proj_pts[i]
...
def main():
    ...
    ap.add_argument("--bg_image", type=str, default="", help="optional background image for heatmap")
    ap.add_argument("--bg_rotate", type=int, default=0, choices=[0,90,180,270], help="rotate bg image if needed")
    args = ap.parse_args()
    ...
    if Path(args.calib).exists():
        ...
        img_pts = pick_points(first, 4, "Click table corners: TL, TR, BR, BL (Enter confirms, r=reset, u=undo)")
        Path(args.calib).write_text(json.dumps({"img_pts": img_pts.tolist()}), encoding="utf-8")
        cap.set(cv2.CAP_PROP_POS_FRAMES, 0)

    Hmat = compute_homography(img_pts)

    # Kurzes Overlay zur Bestätigung (~2s)
    ...
    overlay_frames = int((FPS or 30.0) * 2.0)

    # YOLO model
    det = YOLO(args.weights)

    # Preview writer (optional)
    writer = None
    ...
    try:
        while True:
            ...
            for r in results:
                ...
                x1, y1, x2, y2 = map(int, r.boxes.xyxy[j].tolist())
    ...
    if not img_pts_list:
        print("No ball detections collected. Writing empty heatmap/CSV.")
    track = [(int(f), float(x), float(y)) for f,(x,y) in zip(frames_list, img_pts_list)]
    proj_pts_arr = np.array(proj_pts_list, dtype=float) if proj_pts_list else np.zeros((0,2), float)
    confs = np.array(confs_list, dtype=float) if confs_list else np.zeros((0,), float)
    ...
    # Heatmap (mit optionalem Hintergrundbild)
    import matplotlib
    matplotlib.use("Agg")
    import matplotlib.pyplot as plt

    def load_bg(bg_path: str, rotate: int = 0):
        if not bg_path:
            return None
    ...
    bg = load_bg(args.bg_image, args.bg_rotate)

    fig = plt.figure(figsize=(6, 3.5))
    ax = plt.gca()

    if bg is not None:
        ax.imshow(bg, origin="lower", extent=[0, TABLE_W, 0, TABLE_H], aspect='auto', zorder=0)
    ...
    if bounces:
        ...
        H2 = H2.T
        im = ax.imshow(H2, origin="lower", extent=[0, TABLE_W, 0, TABLE_H],
                       aspect='auto', alpha=0.65, zorder=1)
        ax.scatter(xs, ys, s=10, c="white", edgecolors="black", linewidths=0.3, zorder=2)
        plt.colorbar(im, ax=ax, fraction=0.046, pad=0.04, label="hits per bin")
